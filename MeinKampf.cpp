...
void mouseMove(GLFWwindow* window, double xpos, double ypos) {
    ...
    cam.front.x = cos(glm::radians(yaw)) * cos(glm::radians(pitch));
    cam.front.y = sin(glm::radians(pitch));
    cam.front.z = sin(glm::radians(yaw)) * cos(glm::radians(pitch));
...
#pragma endregion
    ...
    float lasttime = 0.0f;
    chunk TESTCHUNK;
    TESTCHUNK.BLOCKS.fill(STONE);
    TESTCHUNK.FillBlocks({ 0,0,0 }, {15,0,15}, BEDROCK);
    TESTCHUNK.Generate();
...
void movement(float deltaTime) {
    if (glfwGetKey(window, GLFW_KEY_W)) {
        cam.position += glm::normalize(cam.front) * deltaTime * cam.SPEED;
    }
    ...
    if (glfwGetKey(window, GLFW_KEY_A)) {
        ...
        cam.position -= glm::normalize(camRight) * deltaTime * cam.SPEED;
    }
